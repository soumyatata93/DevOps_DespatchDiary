...
class DespatchDiaryController extends Controller
{
    ...
    //     $result= $BcUpdateLocalDbController->compareData();
        return $this->callAPIRequestForOrders($selected_branch_id, $weekStartDate, $weekEndDate,$selectedDate);
    }
    /*get data from view branch id,selected date*/
   
    public function callAPIRequestForOrders($selected_branch_id, $weekStartDate, $weekEndDate,$selectedDate)
    {
        ...
        try{
            ...
            */
            ...
            $dayDeliverySheetRecords=$this->dayPrintDeliverySheet($allOrdersForSelectedWeek,$selectedDate);
        }catch (\Exception $e) {
            die("Could not connect to the database. Please check your configuration. error:" . $e);
            $this->error_msg='Could not connect to the database' . $e ;
        } 
        return response()->json([
            'selectedDatePrintDeliverySheet'=>$dayDeliverySheetRecords,
    ...
    /**
     * Day print delivery sheet
     */
    public function dayPrintDeliverySheet($selectedWeekData,$selectedDate){
        
        $selectedDate = date('Y-m-d', strtotime($selectedDate));
        $newSelectedDateFormat = new Carbon($selectedDate);
        $finalDate = $newSelectedDateFormat->format('l');
        foreach ($selectedWeekData as $key => &$order) { 
            // Check if the van number without run
            if (!Str::contains($key, 'RUN')) {
                // Remove the entire order if there is no run for the specified van number
                unset($selectedWeekData[$key]);
            } else {
                foreach ($order as $day => $data) {
                    if ($day != $finalDate) {
                        unset($order[$day]);
                    }
                }
